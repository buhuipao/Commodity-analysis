...
def Proxy(page):
    ...
    for item in items:
        ...
        if not ip or not port or speed[0] != 'bar_inner fast' or ip_type[0].find('socks') != -1:
            continue
...
def Check(proxy):
    ...
    proxies = {'http': 'http://'+proxy[0]+':'+proxy[1], 'https': 'http://'+proxy[0]+':'+proxy[1]}
    try:
